...
chrome.runtime.onMessage.addListener((msg) => {
  if (msg.action === 'identify-image')  _handleImage(msg.srcUrl).catch(e  => _showError(`Error: ${e.message}`));
  if (msg.action === 'identify-region') _handleRegion(msg.srcUrl).catch(e => _showError(`Error: ${e.message}`));
  if (msg.action === 'identify-page')   _handlePage().catch(e             => _showError(`Error: ${e.message}`));
  if (msg.action === 'detect-ai-text')  _handleAiDetect(msg.srcUrl).catch(e => _showError(`Error: ${e.message}`));
  if (msg.action === 'forensics-image') _handleForensics(msg.srcUrl).catch(e => _showError(`Error: ${e.message}`));
});
...
function _showRegionSelector() {
  return new Promise((resolve) => {
    ...
    Object.assign(banner.style, {
      ...
      borderBottom: '1px solid #4f46e5', pointerEvents: 'none',
    });
...
async function _handleRegion(_srcUrl) {
  ...
  if (!sel) return; // user pressed Esc
  ...
  try {
    ...
    if (error) return _showError(`Screenshot failed: ${error}`);
